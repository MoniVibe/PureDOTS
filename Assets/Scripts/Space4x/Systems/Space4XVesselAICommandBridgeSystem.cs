    ...
    public partial struct Space4XVesselAICommandBridgeSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            for (int i = 0; i < commands.Length; i++)
            {
                ...
                if (!_minerVesselLookup.HasComponent(command.Agent))
                {
                    continue;
                }

                var vessel = _minerVesselLookup[command.Agent];
        ...
        private VesselAIState.Goal MapActionToGoal(Entity agent, byte actionIndex, MinerVessel vessel, ref SystemState state)
        {
            ...
            if (_utilityBindingLookup.HasComponent(agent))
            {
                ...
                if (actionIndex < binding.Goals.Length)
                {
                    ...
                    if (vessel.Load >= vessel.Capacity * 0.95f && goal == VesselAIState.Goal.Mining)
                    {
                        return VesselAIState.Goal.Returning;
                    }
                    
                    // If vessel is not full and goal is Returning, allow Mining
                    if (vessel.Load < vessel.Capacity * 0.95f && goal == VesselAIState.Goal.Returning)
                    {
            ...
            switch (actionIndex)
            {
                case 0:
                    ...
                    if (vessel.Load < vessel.Capacity * 0.95f)
                    {
                        return VesselAIState.Goal.Mining;
                    }
                    return VesselAIState.Goal.Returning;
