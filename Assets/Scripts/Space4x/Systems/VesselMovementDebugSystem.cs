...
namespace Space4X.Systems
{
    ...
    public partial class VesselMovementDebugSystem : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
            ...
            var vesselCount = 0;
            foreach (var (transform, aiState, movement, vessel, entity) in SystemAPI.Query<RefRO<LocalTransform>, RefRO<VesselAIState>, RefRO<VesselMovement>, RefRO<MinerVessel>>()
                .WithEntityAccess())
            {
                vesselCount++;
                ...
                if (vesselCount <= 2) // Log first 2 vessels in detail
                {
                    ...
                    Debug.Log($"[VesselMovementDebug] Vessel {vesselCount}: " +
                             ...
                             $"Load={vessel.ValueRO.Load}/{vessel.ValueRO.Capacity}");
                }
            }
