...
namespace PureDOTS.Systems.Anatomy
{
    ...
    public partial struct BodyPartDamageSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
                         ...
                         .WithEntityAccess())
            {
                ...
                for (int i = 0; i < damageBuffer.Length; i++)
                {
                    ...
                    var partIndex = FindPartIndex(ref partsBuffer, in damageEvent.PartId);
                    if (partIndex < 0)
                    {
                        if (hasHealth)
                        {
        ...
        [BurstCompile]
        private static int FindPartIndex(ref DynamicBuffer<BodyPartState> parts, in FixedString64Bytes partId)
        {
